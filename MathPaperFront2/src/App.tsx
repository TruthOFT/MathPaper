...
type ActiveView = 'dashboard' | 'calculator' | 'questions' | 'papers' | 'tasks' | 'wrongQuestions'
...
const navItems: MenuProps['items'] = [
    ...
    {key: 'wrongQuestions', icon: <ExclamationCircleOutlined/>, label: '错题本'},
]

const viewMeta: Record<ActiveView, { title: string; subtitle: string }> = {
    dashboard: {
        title: '数据看板',
        subtitle: '汇总作业、题库、成绩和薄弱知识点，先看全局再处理细节。',
    },
    calculator: {
        title: '公式计算',
    ...
    wrongQuestions: {
        title: '错题本',
        subtitle: '集中复习错题，查看标准答案和解析，掌握后标记完成。',
    },
}
...
function App() {
    ...
    const allowedNavItems = navItems?.filter((item) => {
        ...
        if (isTeacher && item?.key === 'wrongQuestions') {
            return false
        }
        return true
    })
    const selectedView = isTeacher && activeView === 'wrongQuestions'
        ? 'dashboard'
        : !isTeacher && (activeView === 'questions' || activeView === 'papers')
        ? 'tasks'
        : activeView
        ...
        <Layout className="app-shell">
            ...
            <Layout className="main-layout">
                ...
                <Content className="content-wrap">
                    ...
                    {selectedView === 'wrongQuestions' && <WrongQuestionsPanel/>}
                </Content>
            </Layout>
        </Layout>
    )
...
function DashboardPanel() {
    ...
    const load = useCallback(async () => {
        setLoading(true)
        try {
            setSummary(await request<DashboardSummary>('/api/dashboard/summary'))
        } catch (error) {
...
function TaskPanel() {
    ...
    const openTaskSummary = async (task: TaskSummary) => {
        ...
        setLoading(true)
        try {
            setTaskStudents(await request<TaskStudent[]>(`/api/tasks/${task.taskId}/students`))
        } catch (error) {
...
function WrongQuestionsPanel() {
    ...
    const load = useCallback(async () => {
        setLoading(true)
        try {
            setItems(await request<WrongQuestion[]>('/api/wrong-questions/mine'))
        } catch (error) {
    ...
    const review = async (item: WrongQuestion) => {
        setLoading(true)
        try {
            await request<WrongQuestion>(`/api/wrong-questions/${item.id}/review`, {method: 'POST'})
    ...
    const mastered = async (item: WrongQuestion) => {
        setLoading(true)
        try {
            await request<WrongQuestion>(`/api/wrong-questions/${item.id}/mastered`, {method: 'POST'})
